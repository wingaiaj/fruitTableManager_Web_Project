...
public class FruitDAOImp extends BaseDAO<Fruit> implements FruitDAO {
    ...
    public int getFruitCount() {
        ...
        try {
            ...
            connection = jdbcUtils.getConnection();
            //调用intValue转换为int类型
            return ((Long)getCount(connection, sql)[0]).intValue();
            //关连接
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    @Override
    public List<Fruit> getFruitLimit(Integer num) {
        Connection connection = null;
        try {
            connection = jdbcUtils.getConnection();
            String sql = "select fid,fname as 'fName',price,fCount as 'count',remark from fruit limit ?,8";
            return getListRecord(connection, sql, (num - 1) * 8);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            jdbcUtils.close(connection, null, null);
